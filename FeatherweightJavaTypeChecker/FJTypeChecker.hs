...
-- Get class declaration from the class table
...
-- Check if a class is subtype of another
...
-- The two following functions can be used in future work for the use of interfaces
...
-- Get fields extended from super class
...
-- Check if a class has a given field
findField :: String -> [(Ty, String)] -> Bool
findField s [] = False
findField s ((_, b):xs) | s == b    = True
                        | otherwise = findField s xs

-- Get list of types from list of pairs (Type ,String)
...
-- Check if all the typings of variables are correct
...
-- Check if every variable on an operation is of type Int
...
-- Add binding of variable given in a declaration
...
-- Check the typing of the method's body
...
-- Main function to verify typing of program
...
-- Function to start the type checking of program
...
-- Generate the class declaration list
getCT :: ClassDeclaration -> ClassTable
getCT cls@(Class c d a (Cons s ts ls lss) m) = [(c, cls)]

-- Function to start type checking after building class table and context
